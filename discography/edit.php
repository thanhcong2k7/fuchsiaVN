...
if (!empty($release->relDate) && $release->relDate !== '0000-00-00') { // Check for valid date
    ...
    } catch (Exception $e) { $relDateFormatted = ''; } // Handle invalid date format
}
$orgRelDateFormatted = '';
if (!empty($release->orgReldate) && $release->orgReldate !== '0000-00-00') { // Check for valid date
    ...
    } catch (Exception $e) { $orgRelDateFormatted = ''; } // Handle invalid date format
}
...
<head>
    ...
    <link rel="stylesheet" href="/assets/css/select2_custom.css"> <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.3.0/css/datepicker3.css" />
    ...
    <style>
        ...
        /* DnD Artwork */
        .dnd { width: 100%; height: 100%; display: flex; justify-content: center; }
        #drop-area { width: 300px; height: 300px; padding: 15px; text-align: center; border-radius: 7px; border: 2px dashed #ffffff; cursor: pointer; }
        #img-view { width: 100%; height: 100%; border-radius: 7px; border: 1px dashed #AFFFFFFF; background: #BF000000; background-position: center; background-size: cover; display: flex; align-items: center; justify-content: center; flex-direction: column; }
        #img-view img { width: 100px; /* Adjust icon size */ margin-bottom: 15px; } /* Style the icon */
        #img-view span { display: block; font-size: 12px; color: #eeeeee; line-height: 1.4; }

        /* Loader */
        .loader { width: 30px; aspect-ratio: 1; --c: no-repeat linear-gradient(#fff 0 0); background: var(--c) 0% 50%, var(--c) 50% 50%, var(--c) 100% 50%; background-size: 20% 100%; animation: l1 1s infinite linear; }
        @keyframes l1 { 0%, 100% { background-size: 20% 100%, 20% 100%, 20% 100% } 33% { background-size: 20% 10%, 20% 100%, 20% 100% } 50% { background-size: 20% 100%, 20% 10%, 20% 100% } 66% { background-size: 20% 100%, 20% 100%, 20% 10% } }

        /* Modal */
        .modal-content { background-color: rgba(0, 0, 0, 0.9) !important; color: #fff; font-size: 11px; }
        .modal-header, .modal-footer { border-color: #333 !important; }
        .modal-header .close { color: #fff !important; opacity: 1; }

        /* Alert */
        .alert.callout { padding: 15px; border-color: #6DD134; border-radius: 5px; border-width: 2px; color: white; }

        /* Select2 Dark Theme Adjustments */
        .select2-container--default .select2-selection--multiple { background-color: #3A3F51; border: 1px solid #545A75; color: #fff; } /* Adjust background and border */
        .select2-container--default .select2-selection--multiple .select2-selection__rendered { color: #fff !important; } /* List color */
        .select2-container--default .select2-selection--multiple .select2-selection__choice { background-color: #545A75; border: 1px solid #6C757D; color: #fff; } /* Choice background */
        .select2-container--default .select2-selection--multiple .select2-selection__choice__remove { color: #ced4da; } /* Choice remove icon */
        .select2-container--default .select2-search--inline .select2-search__field { color: #fff !important; } /* Search input text */
        /* Dropdown Styles */
        .select2-dropdown { background-color: #3A3F51; border: 1px solid #545A75; color: #fff;}
        .select2-container--default .select2-results__option { color: #fff !important; }
        .select2-container--default .select2-results__option--highlighted[aria-selected] { background-color: #545A75 !important; color: #fff !important; }
        .select2-container--default .select2-results__option[aria-selected=true] { background-color: #6C757D; } /* Selected item in dropdown */
        .select2-results__option--selectable { color: #fff; }
    </style>
</head>
...
<body class="bg-theme bg-theme1">
                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="table-responsive overflow-hidden tab-content p-3">
                                        ...
                                        <div class="tab-pane active card mb-0" id="metadata"> <div class="card-header"> <i class="zmdi zmdi-info"></i> Album Metadata
                                            </div>
                                            <div class="card-body"> <div class="row">
                                                    <div class="col-md-4"> <div class="dnd">
                                                            <label for="input-file" id="drop-area">
                                                                <input type="file" accept="image/*" id="input-file" name="artworkup" hidden>
                                                                <div id="img-view" style="<?php echo !empty($release->art) ? 'background-image: url(\''.htmlspecialchars($release->art, ENT_QUOTES, 'UTF-8').'\');' : ''; ?>">
                                                                     <?php if (empty($release->art)): ?>
                                                                         <i class="zmdi zmdi-file-plus" style="font-size: 50px; margin-bottom: 15px;"></i>
                                                                         <span id="texttt">Drag & Drop or Click to Upload Artwork<br>(Min. 1500x1500, Square)</span>
                                                                     <?php endif; ?>
                                                                </div>
                                                            </label>
                                                        </div>
                                                    ...
                                                    <div class="col-md-8" style="padding-top: 20px;">
                                                        ...
                                                        <span><span style="font-weight: bold;">Status:
                                                                ...
                                                                switch ($release->status) {
                                                                    ...
                                                                    case 1: $statusText = "DELIVERED"; break;
                                                                    case 2: $statusText = "ERROR"; break;
                                                                    case 3: $statusText = "CHECKING"; break;
                                                                }
                                                                echo $statusText;
                                                                ?></span>
                                                        </div>
                                                ...
                                                <br>
                                                <div class="row">
                                                    <div class="form-group col-md-6"> <label for="albumtitle">Album Title</label>
                                                    ...
                                                    <div class="form-group col-md-6"> <label for="albumversion">Version line (optional)</label>
                                                ...
                                                <div class="row">
                                                    <div class="form-group col-md-6" id="sandbox-container">
                                                        ...
                                                        <div class="input-group"> <input type="text" class="form-control" id="reldate" name="reldate"
                                                    ...
                                                    <div class="form-group col-md-6" id="sandbox-container2">
                                                        <label for="orgreldate">Original release date (optional, mm/dd/yyyy)</label>
                                                         <div class="input-group"> <input type="text" class="form-control" id="orgreldate" name="orgreldate"
                                                ...
                                                <hr class="mt-1 mb-3" /> <div class="row">
                                                    <div class="form-group col-12"> <label for="upc">UPC (optional)</label>
                                                        <input type="text" class="form-control" maxlength="12" name="upc" pattern="\d{12}" title="Enter a 12-digit UPC"
                                                ...
                                                <div class="row">
                                                    <div class="form-group col-md-4">
                                                        ...
                                                        <input type="number" class="form-control" name="cyear" pattern="\d{4}" title="Enter a 4-digit year"
                                                            placeholder="<?php echo $currentYear; ?>"
                                                            value="<?php echo htmlspecialchars($release->cyear ?? $currentYear, ENT_QUOTES, 'UTF-8'); ?>" required>
                                                    </div>
                                                    <div class="form-group col-md-8">
                                                        ...
                                                        <input type="text" class="form-control" name="cline"
                                                            ...
                                                            value="<?php echo htmlspecialchars($release->c ?? '', ENT_QUOTES, 'UTF-8'); ?>" required>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="form-group col-md-4">
                                                        <label for="pyear">℗ Phonogram Year</label>
                                                        <input type="number" class="form-control" name="pyear" pattern="\d{4}" title="Enter a 4-digit year"
                                                            placeholder="<?php echo $currentYear; ?>"
                                                            value="<?php echo htmlspecialchars($release->pyear ?? $currentYear, ENT_QUOTES, 'UTF-8'); ?>" required>
                                                    </div>
                                                    <div class="form-group col-md-8">
                                                        ...
                                                        <input type="text" class="form-control" name="pline"
                                                            ...
                                                            value="<?php echo htmlspecialchars($release->p ?? '', ENT_QUOTES, 'UTF-8'); ?>" required>
                                                    </div>
                                                </div>
                                            </div> </div> <div class="tab-pane card mb-0" id="tracks">
                                            <div class="card-header">
                                                <i class="zmdi zmdi-playlist-audio"></i> Tracks List
                                                <div class="card-action">
                                                    <div class="dropdown">
                                                        <a href="#" class="text dropdown-toggle dropdown-toggle-nocaret" data-toggle="modal" data-target="#addTrackModal"> <i class="zmdi zmdi-collection-plus"></i> Add Tracks
                                                        </a>
                                                    </div>
                                                </div>
                                            </div>
                                                        ...
                                                        <?php if (!empty($track)): ?>
                                                            <?php foreach ($track as $tr): ?>
                                                                <tr id="track<?php echo $tr->id; ?>">
                                                                    ...
                                                                    <td><?php echo htmlspecialchars($tr->name ?? 'N/A', ENT_QUOTES, 'UTF-8'); ?></td>
                                                                    <td><?php echo $tr->artistname; // Already escaped ?></td>
                                                                    <td>
                                                                        <a href="#" class="text-warning delete-track" data-track-id="<?php echo $tr->id; ?>" data-album-id="<?php echo $releaseId; ?>">
                                                                            Delete
                                                                        </a>
                                                                    </td>
                                                                </tr>
                                                            <?php endforeach; ?>
                                                         <?php else: ?>
                                                            <tr><td colspan="4" class="text-center">No tracks added to this release yet.</td></tr>
                                                         <?php endif; ?>
                                                    </tbody>
                                                </table>
                                            </div> </div> <div class="tab-pane card mb-0" id="dist">
                                             <div class="card-header">
                                            ...
                                            <div class="card-body">
                                                <div class="row">
                                                    <div class="col-12">
                                                         ...
                                                         <div class="input-group select2-bootstrap-append">
                                                             <select name="stores[]" class="form-control" id="stores" multiple data-placeholder="Choose stores/services to distribute to" style="width:100%;">
                                                                 <?php
                                                             ...
                                                             <div class="input-group-append">
                                                                <span class="input-group-text">
                                                                    <input type="checkbox" id="toggleSelectAllStores" title="Select/Deselect All">
                                                                </span>
                                                             </div>
                                                         </div>
                                                    </div>
                                                ...
                                                <div class="row">
                                                    <div class="col-md-6"> <div class="alert alert-warning callout mt-3" role="alert"> <i class="zmdi zmdi-info-outline"></i> <strong>Note:</strong> Enable YouTube Content ID only if your track is 100% original and contains no third-party samples/loops/sounds unless properly licensed for Content ID usage. Misuse can lead to penalties.
                                                        </div>
                                                        <div class="card card-body mb-2"> <div class="icheck-material-white">
                                                                <input type="checkbox" id="ytcid" name="ytcid" value="1" <?php echo !empty($release->ytcid) ? "checked" : ""; ?> />
                                                                <label for="ytcid"> YouTube Content ID</label>
                                                            </div>
                                                        </div>
                                                        <div class="card card-body mb-2">
                                                            <div class="icheck-material-white">
                                                                <input type="checkbox" id="scloud" name="scloud" value="1" <?php echo !empty($release->sc) ? "checked" : ""; ?> />
                                                                <label for="scloud" style="overflow: hidden;white-space: initial;"> SoundCloud Monetization & Content Protection</label>
                                                            </div>
                                                        </div>
                                                          <div class="card card-body mb-2">
                                                            <div class="icheck-material-white">
                                                                <input type="checkbox" id="soundx" name="soundx" value="1" <?php echo !empty($release->sx) ? "checked" : ""; ?> />
                                                                <label for="soundx"> SoundExchange Registration</label>
                                                            </div>
                                                        </div>
                                                    </div>

                                                    <div class="col-md-6"> <div class="card card-body mb-2">
                                                            <div class="icheck-material-white">
                                                                <input type="checkbox" id="jdl" name="jdl" value="1" <?php echo !empty($release->jdl) ? "checked" : ""; ?> />
                                                                <label for="jdl"> Juno Download</label>
                                                            </div>
                                                        </div>
                                                        <div class="card card-body mb-2">
                                                            <div class="icheck-material-white">
                                                                <input type="checkbox" id="trl" name="trl" value="1" <?php echo !empty($release->trl) ? "checked" : ""; ?> />
                                                                <label for="trl"> Tracklib</label>
                                                            </div>
                                                        </div>
                                                        <div class="card card-body mb-2">
                                                            <div class="row align-items-center no-gutters"> <div class="col-auto pr-2">
                                                                    <div class="icheck-material-white">
                                                                        <input type="checkbox" id="bport" name="bport_enabled" value="1" <?php echo !empty($release->bp) ? "checked" : ""; ?> /> <label for="bport"> Beatport</label>
                                                                    </div>
                                                                </div>
                                                                <div class="col">
                                                                     <input type="text" class="form-control form-control-sm" name="bporturl" placeholder="Label Page URL (optional, e.g., beatport.com/label/your-label/xxxxx)" value="<?php echo htmlspecialchars($release->bp ?? '', ENT_QUOTES, 'UTF-8'); ?>">
                                                                </div>
                                                             </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            </div> </div> </div> <div class="text-center mt-4"> <input name="saveform" id="saveform" type="submit" class="btn btn-light btn-round px-5" value="Save Changes">
                                        <input name="distform" id="distform" type="submit" class="btn btn-warning btn-round px-5" value="Distribute Now"> </div>

                                </div> </div> </form> </div>
                </div>
                <div class="overlay toggle-menu"></div>
                </div>
            </div><div class="modal fade" id="addTrackModal" tabindex="-1" role="dialog" aria-labelledby="addTrackModalTitle" aria-hidden="true"> <div class="modal-dialog modal-lg" role="document"> <div class="modal-content">
                    ...
                    <div class="modal-body">
                         <div id="catalogueTrackList">
                              <div class="text-center p-3"><div class="loader"></div> Loading catalogue...</div>
                         </div>
                         </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary px-5" data-dismiss="modal">Close</button> <button type="button" id="addSelectedTracksBtn" class="btn btn-primary px-5">Add Selected Tracks</button> </div>
                </div>
            </div>
        </div>
    ...
    <script src="../assets/plugins/jquery-loading-indicator-master/dist/jquery.loading-indicator.js"></script> <script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.3.0/js/bootstrap-datepicker.js"></script>
