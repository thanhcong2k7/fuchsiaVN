...
function getStore($status)
{
	$tmp = query("select * from stores " . ($status ? "where status=1" : "") . ";");
	$tmp2 = array();
...
function getRelease($uid, $num = 0, $id = 0)
{
	...
	while ($row = $tmp1->fetch_assoc()) {
		...
		$tmp5 = "[]";
		$tmp6 = query("select * from storereq where id=" . strval($row["albumID"]) . ";");
		while ($row2 = $tmp6->fetch_assoc())
			$tmp5 = $row2["data"];
		$tmp2->store = json_decode($tmp5);
...
function getAllReleases($statusFilter = null) // Optional status filter (e.g., 'pending', 'approved')
{
	...
	if ($tmp1) {
		while ($row = $tmp1->fetch_assoc()) {
			...
			//$tmp2->store = json_decode($row["storeID"]);
			$tmp5 = "[]";
			$tmp6 = query("select * from storereq where id='" . $row["albumID"] . "';");
			while ($row2 = $tmp6->fetch_assoc())
				$tmp5 = $row2["data"];
			$tmp2->store = json_decode($tmp5);
